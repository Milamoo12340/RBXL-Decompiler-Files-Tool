...
function TopicDetailView({ sessionId, topicId }: { sessionId: number, topicId: number }) {
  const { data: detail, isLoading } = useGetTopicDetail(sessionId, topicId, {
    query: { enabled: !!topicId, queryKey: getGetTopicDetailQueryKey(sessionId, topicId) }
  });
    ...
    <div className="flex flex-col h-full bg-background relative overflow-hidden">
      <div className="h-16 flex flex-col justify-center px-6 border-b border-border bg-card/30 shrink-0">
        ...
        <p className="text-xs text-muted-foreground font-mono">
          FOUND {detail.matchCount} RELEVANT {detail.matchCount === 1 ? 'SNIPPET' : 'SNIPPETS'}
        </p>
      </div>
      ...
      <ScrollArea className="flex-1 p-6">
        <div className="flex flex-col gap-6 max-w-4xl mx-auto pb-12">
          {detail.findings.map((finding, idx) => (
            <div key={`${finding.scriptId}-${idx}`} className="flex flex-col border border-border rounded-md overflow-hidden bg-card/50 hover:border-primary/50 transition-colors">
              ...
              <div className="p-3 bg-[#1d1f21] overflow-x-auto text-sm font-mono text-muted-foreground leading-relaxed">
                <pre className="m-0">
                  <code className="text-[13px]">{finding.snippet}</code>
                </pre>
              </div>
            </div>
          ))}
...
export default function SessionAnalysis() {
  ...
  return (
    <div className="min-h-[100dvh] flex flex-col bg-background h-screen overflow-hidden">
      <SessionHeader sessionId={sessionId} sessionName={session.originalName} activeTab="analysis" />

      <main className="flex-1 flex overflow-hidden">
        <ResizablePanelGroup direction="horizontal">
          <ResizablePanel defaultSize={30} minSize={20} maxSize={45} className="flex flex-col bg-card/30 border-r border-border">
            <div className="p-4 border-b border-border shrink-0">
              <h2 className="text-sm font-bold flex items-center gap-2 uppercase tracking-widest text-muted-foreground">
                <Activity className="w-4 h-4 text-primary" />
                Discovered Topics {analysis && <span className="text-xs text-primary/70">({analysis.topicCount})</span>}
              </h2>
            </div>
                  ...
                  topics?.map(topic => (
                    ...
                    >
                      <div className="flex items-center justify-between w-full">
                        <span className="font-semibold text-sm truncate pr-2">{topic.name}</span>
                        <Badge variant="outline" className="text-[10px] px-1 h-4 bg-background shrink-0">
                          {topic.matchCount}
                        </Badge>
                      </div>
          ...
          <ResizablePanel defaultSize={70}>
            {selectedTopicId ? (
              <TopicDetailView sessionId={sessionId} topicId={selectedTopicId} />
            ) : (
              <div className="flex flex-col items-center justify-center h-full text-muted-foreground opacity-50">
                <Code2 className="w-16 h-16 mb-4 opacity-50" />
                <h3 className="text-lg font-mono font-bold tracking-widest">AWAITING_SELECTION</h3>
                <p className="text-sm">Select an analysis topic to explore extracted mechanics.</p>
              </div>
            )}
          </ResizablePanel>
        </ResizablePanelGroup>
